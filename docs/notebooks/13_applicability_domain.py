...
knn_ad.fit_threshold(
    binary_fp_pipe.named_steps["fp"].transform(X_val), target_percentile=95
)
...
X_val_transformed = count_fp_pipe[
    ...
    X_val
)  # Index into pipeline to get all the pipeline up to thelast step before the AD estimator
count_fp_pipe.named_steps["leverage"].fit_threshold(
    X_val_transformed, target_percentile=95
)


# Get AD scores for test set
X_test_transformed = count_fp_pipe[:-1].transform(
    X_test
)  # Index into pipeline to get the last step before the AD estimator
leverage_raw_scores = count_fp_pipe.named_steps["leverage"].transform(
    X_test_transformed
)
...
plt.axvline(
    x=count_fp_pipe.named_steps["leverage"].threshold_,
    color="r",
    linestyle="--",
    label="AD Threshold",
)
...
Draw.MolsToGridImage(
    [Chem.MolFromSmiles(drug) for drug in famous_drugs.values()],
    molsPerRow=3,
    subImgSize=(250, 250),
    legends=[f"{name}" for name, smiles in famous_drugs.items()],
)
